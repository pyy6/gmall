...
public class ConsumerXie {
    public static void main(String[] args) {
        ...
        try {
            ...
            Destination testqueue = session.createQueue("DAOSHUI");//监听倒水队列,监听消息的时候也需要看监听消息的类型和监听消息的名称
            ...
            consumer.setMessageListener(new MessageListener() {//时时刻刻监听消息，从消息队列拖拽消息进行处理
                ...
                public void onMessage(Message message) {
                    if(message instanceof TextMessage){
                        try {
                            String text = ((TextMessage) message).getText();//根据消息队列里面消息的类型强转
                            System.out.println(text+"Xie");
